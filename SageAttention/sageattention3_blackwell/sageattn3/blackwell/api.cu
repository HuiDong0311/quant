...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x (head_size // 2)
    ...
    // --- Device info for FP4 kernel selection ---
    if (strstr(dprops.name, "B100") || strstr(dprops.name, "B200") ||
        strstr(dprops.name, "GB200") || strstr(dprops.name, "5090") ||
        strstr(dprops.name, "Blackwell")) {
        printf("[INFO] Detected %s (reported %d.%d); using SM120 (Blackwell) FP4 kernel\n",
            dprops.name, dprops.major, dprops.minor);
    } else {
        // 老卡就当普通支持路径处理，不再误报强制警告
        printf("[INFO] FP4 kernel on GPU %s (reported %d.%d)\n",
            dprops.name, dprops.major, dprops.minor);
    }
